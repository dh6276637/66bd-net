...
# 智能修改render_template - 找到article_detail的render_template
# 使用正则匹配多行render_template
old_pattern = r"render_template\('article_detail\.html',\s*article=article"
...
# 检查是否有article_detail的render_template
if re.search(old_pattern, new_content):
    # 找到匹配位置
    match = re.search(old_pattern, new_content)
    start = match.start()
    # 找到这个render_template调用的结束 - 匹配括号
    paren_count = 0
    in_call = False
    end = start
    for i, c in enumerate(new_content[start:], start):
        if c == '(':
            paren_count += 1
            in_call = True
        elif c == ')':
    ...
    # 获取完整调用
    full_call = new_content[start:end]
    # 添加新参数到article=article之后
    if 'content_html=' not in full_call:
        modified_call = full_call.replace('article=article', 
            "article=article, content_html=text_to_html(article.content if article.content else ''), content_cn_html=text_to_html(article.content_cn) if article.content_cn else ''")
        new_content = new_content[:start] + modified_call + new_content[end:]
